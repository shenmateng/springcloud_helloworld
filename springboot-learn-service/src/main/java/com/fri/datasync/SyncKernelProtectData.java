...
/**
 * @author : 陈余银
 * @create : 2022-05-30 16:54:00
 * @description :
 */
public class SyncKernelProtectData {
    public static void main(String[] args) throws Exception {
        ...
        MySqlSource<String> sourceFunction = MySqlSource.<String>builder()
                ...
                .databaseList(FlinkNacosConfig.getConfigValue("mysql.kernelProtect.database"))
                .tableList(FlinkNacosConfig.getConfigValue("mysql.kernelProtect.table"))
                .username(username)
                .password(password)
        ...
        DataStreamSource<String> commonSource = env
                .fromSource(sourceFunction, WatermarkStrategy.noWatermarks(), "sync ys_kernel_protect source");

        commonSource
                .addSink(new MysqlSinkPlus(FlinkNacosConfig.getConfigValue("mysql.kernelProtect.url"), username, password, driverClassName))
                .setParallelism(1);

        env.execute("Sync KernelProtect DB Data");
    }
}
