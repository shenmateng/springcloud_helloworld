...
public class SyncUserData {
    public static void main(String[] args) throws Exception {
        ...
        MySqlSource<String> sourceFunction = MySqlSource.<String>builder()
                ...
                .databaseList(FlinkNacosConfig.getConfigValue("mysql.user.database"))
                .tableList(FlinkNacosConfig.getConfigValue("mysql.user.table"))
                .username(username)
                .password(password)
        ...
        DataStreamSource<String> commonSource = env
                .fromSource(sourceFunction, WatermarkStrategy.noWatermarks(), "sync ys_user source");

        commonSource
                .addSink(new MysqlSinkPlus(FlinkNacosConfig.getConfigValue("mysql.user.url"), username, password, driverClassName))
                .setParallelism(1);

        env.execute("Sync User DB Data");
    }
}
